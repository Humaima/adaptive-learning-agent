...
def build_nodes(cg: ConceptGraph):
    """
    Returns node functions closing over the concept graph only (static, safe to share).
    Each node opens its own DB session per call — safe across many HTTP requests
    hitting the same long-lived graph object.
    """
    ...
    def collect_answers_node(state: TutorGraphState) -> dict:
        ...
        student_answers = interrupt(payload)
    ...
    def update_node(state: TutorGraphState) -> dict:
        ...
        try:
            result = run_update_loop(
                db, student_id, original_question,
                target_concept_id, cg, eval_results,
            )
        finally:
            db.close()
        return {
    ...
    return {
        "assess": assess_node, "difficulty": difficulty_node, "tutor": tutor_node,
        "quiz": quiz_node, "collect_answers": collect_answers_node,
        "evaluate": evaluate_node, "update": update_node,
    }
...
def route_after_update(state: TutorGraphState) -> str:
    ...
    target_assessment = next(a for a in assessment.assessments if a.concept_id == target_id)
